...
class AudioClassifier:
    def __init__(self, sample_rate=44100, duration=2.0, overlap_factor=0.5):
        ...
        # Setup interpreter
        self.interpreter = tf.lite.Interpreter(model_path="quantized_mobilenetv3_mini.tflite")
    ...
    def infer_from_stream(self):
        ...
        while not self.stop_flag:
            try:
                ...
                buffer[-self.chunk_size:] = np.squeeze(chunk)
    ...
    def run(self):
        ...
        with sd.InputStream(callback=self.audio_callback, channels=1, samplerate=self.sample_rate, blocksize=int(self.chunk_size*self.overlap_factor)) as stream:
